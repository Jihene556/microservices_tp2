...
// A function to get all items of a specific list
export async function getItemsFromList(
  this: any,
  request: FastifyRequest<{ Params: { listId: string } }>,  // Typing for listId param
  reply: FastifyReply
) {
  const { listId } = request.params;  // Extract listId from URL params

  try {
    const listData = await this.level.db.get(listId);  // Fetch the list from the database
    if (!listData) {
      return reply.status(404).send({ message: 'This list does not exist in the database' });
    }

    const list = JSON.parse(listData) as ITodoList;
    // Send the items and the description of the list
    //reply.send({items: list.items });
    reply.send({description: list.description, items: list.items})
  } catch (error) {
    reply.status(500).send({ message: 'Error retrieving the list items' });
  }
}
